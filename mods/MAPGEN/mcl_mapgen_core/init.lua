...
local function world_structure(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	...
	if minp.y <  underworld_bounds.min + 10 or maxp.y <  underworld_bounds.min + 60 then
		...
		ores = {min=underworld_bounds.min - 10,max=underworld_bounds.min + 20}
	end
...
local function block_fixes_grass(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	...
	return minp.y <= overworld_bounds.max and maxp.y >= overworld_bounds.min and
...
local function end_basic(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	if maxp.y < fringe_bounds.min or minp.y > fringe_bounds.max then return end
	local lvm_used = false
