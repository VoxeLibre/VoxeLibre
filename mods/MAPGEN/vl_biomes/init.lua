...
function vl_biomes.register_biome(def)
	...
	if is_overworld then
		...
		if sub._beach then
			sub._beach._mcl_skycolor = sub._beach._mcl_skycolor or vl_biomes.skycolor.beach
		end
		if sub._ocean then
			local odef = sub._ocean
		...
		if sub._ocean and sub._deep_ocean == nil then
			-- TODO: what if min_pos/max_pos are set?
			sub._deep_ocean = {
				y_min = vl_biomes.DEEP_OCEAN_MIN,
				y_max = vl_biomes.DEEP_OCEAN_MAX,
				node_top = sub._ocean.node_top or def.node_top,
				depth_top = 2,
				node_filler = sub._ocean.node_filler or def.node_filler,
				depth_filler = 3,
				node_riverbed = sub._ocean.node_riverbed or def.node_riverbed,
				depth_riverbed = 2,
				vertical_blend = 5,
				_mcl_foliage_palette_index = 0, -- to avoid running the foliage fix
				_mcl_skycolor = vl_biomes.skycolor.ocean,
				_mcl_waterfogcolor = sub._ocean._mcl_waterfogcolor,
			}
		end
		...
		if sub._underground == nil then
			-- TODO: y_min, y_max only if not min_pos, max_pos
			sub._underground = {
				node_top = (sub._ocean and sub._ocean.node_top) or def.node_top,
				depth_top = 2,
				node_filler = (sub._ocean and sub._ocean.node_filler) or def.node_filler,
				depth_filler = 3,
				node_riverbed = (sub._ocean and sub._ocean.node_riverbed) or def.node_riverbed,
				depth_riverbed = 2,
	...
	for k, sdef in pairs(sub) do
		sdef.name = sdef.name or def.name .. k
		-- merge from parent
	...
	-- core.log("action", "registering biome "..tostring(def.name))
	for _, sdef in pairs(sub) do
		-- core.log("action", "registering subbiome "..tostring(sdef.name).." "..dump(sdef,""))
