...
local max, min, abs, sqrt, floor, ceil = math.max, math.min, math.abs, math.sqrt, math.floor, math.ceil
local hash_pos, bitmix32 = mcl_util.hash_pos, mcl_util.bitmix32
...
local function spawn_threshold(def, sidelen)
	...
	if def.hash_mindist_2d then
		...
		local k = in_radius_2d(math.max(1, r / sidelen))
		...
		local k2 = 3 --2 * math.max(1, math.ceil(ry / sidelen)) + 1
