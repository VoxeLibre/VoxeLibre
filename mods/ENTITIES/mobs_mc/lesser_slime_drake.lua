...
--################### LESSER SLIME DRAKE
--###################
-- Slime drake and attack textures, attack phase logic, breath attack, by Thomas Conway (C.2025)
-- Boss Fight music is "Baozi" from Quake Community Music Pack 1 released under CC-BY-SA-4.0
-- Code based on original Ender Dragon from mobs_mc
...
mcl_mobs.register_mob("mobs_mc:lesser_slime_drake", {
	description = S("Lesser Slime Drake"),
	type = "monster",
	spawn_class = "hostile",
	pathfinding = 1,
	...
	textures = {
		{"mobs_mc_lesser_slime_drake.png"},
	},
	visual_size = {x=3, y=3},
	view_range = 64,
	walk_velocity = 6,
	...
	do_custom = function(self, dtime)
		-- Initialize music handle if not set
		if not self._battle_music_handle then
			self._battle_music_handle = nil
		end
		...
		-- Update boss bar
		mcl_bossbars.update_boss(self.object, "Slime Drake", "green")
		...
		if player_in_range and closest_player then
			-- If we didn't have a target before, start the battle music
			if not self.attack and not self._battle_music_handle then
				self._battle_music_handle = minetest.sound_play("dragon-fight-baozi", {
			...
			-- Stop music when no players in range
			if self._battle_music_handle then
				minetest.sound_stop(self._battle_music_handle)
				self._battle_music_handle = nil
			end
			self.attack = nil
...
mcl_mobs.register_egg("mobs_mc:lesser_slime_drake", S("Lesser Slime Drake"), "#252525", "#b313c9", 0)

mcl_mobs:non_spawn_specific("mobs_mc:lesser_slime_drake","overworld",0,minetest.LIGHT_MAX+1)
